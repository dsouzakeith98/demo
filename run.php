...
<body>
 ...
 <div class="belnav">
    ...
    <div class="belnavb modal2-button2">Login</div>

    <a><div class="vl"></div></a>
...
<div class="modal2">
 ...
 <div class="input-block">
        <label for="uname" class="input2-label2">Username</label>
        <input type="textbox" name="uname" id="uname" placeholder="Username">
      </div>



      <div class="input2-block2">
        <label for="password2" class="input2-label2">Password</label>
        <input type="password" name="password2" id="password2" placeholder="Password">
      </div>
  <button type="submit" class="submit2" id="login"name="submit2" value="submit2" class="example_f modal2-button2">Login In</button>
 </div>
...
`
  const closeButton2 = document.querySelector(".close2-button2");
    ...
    let passd=document.getElementById("password2").value;
     let lid=document.getElementById("login");
   
    const modalLeft2 = document.querySelector('.modal2-left2');
        var xmlhttp = new XMLHttpRequest();
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                myFunction(this.responseText);
                if(/error/.test(this.responseText.toLowerCase())){
                  Array.from(document.querySelectorAll('.error2-message2')).map(x => x.remove());
                  const errEl = document.createElement('div');
                  errEl.classList.add('error2-message2');
                  errEl.innerHTML = `<br><small><b><font color="red";>`+this.responseText+`</font></b></small>`
                  modalLeft2.append(errEl)
                  return;
                }
                modalLeft2.innerHTML = successMessage
                          window.setTimeout(function(){

        
        window.location.href = "run2.php";

    }, 1000);
            }
        };
        let param = "&uname="+ username
        +"&password2="+ passd
        +"&login="+lid
        ...
        xmlhttp.open("GET","finallogin.php?"
            + param
            , true);
        xmlhttp.send();
}
...
modalButton2.forEach(btn => btn.addEventListener("click", openModal2));
closeButton2.addEventListener("click", closeModal2);
submit2.addEventListener('click',storeindb2);
  ...
  <div class="modal-container">
    <div class="modal-left">
      ...
      <div class="input-block">
        ...
        <input type="textbox" name="fname" id="fname" placeholder="Full Name" required>
      </div>
      <div class="input-block">
      ...
      <label for="phone" class="input-label">Phone</label>
        ...
        <input type="date" name="dateof" id="dateof" placeholder="" required>
      </div>
...
`
  const closeButton = document.querySelector(".close-button");
    ...
    let compass=document.getElementById("cpassword").value;
    const modalLeft = document.querySelector('.modal-left');
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                myFunction(this.responseText);
                if(/error/.test(this.responseText.toLowerCase())){
                  Array.from(document.querySelectorAll('.error-message')).map(x => x.remove());
                  const errEl = document.createElement('div');
